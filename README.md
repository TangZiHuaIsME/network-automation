...
About · Skills · Structure · Features · Getting Started · Author
...
Quick tasks, custom parsing, learning device interaction
...
Repeatable config management and operational runbooks
...
Concurrent multi-device tasks with structured API data
...
Runs show version, show inventory, show process cpu; saves JSON report
...
Audits interfaces, CPU, memory, OSPF neighbors, hostname, and inventory
...
Backs up running-config to a date-stamped folder
...
Pushes config commands (e.g. NTP, SNMP) to devices
...
Validates required VLANs against show vlan brief
...
Checks OSPF neighbor states; flags non-FULL adjacencies
...
Parses interface rates from show interface; alerts on high usage
...
Polls interface status and logs down/recovery events
...
Collects model, serial, and software version from devices
...
Parses show ip interface brief with TTP into structured JSON
...
Executes show commands and displays output
...
Retrieves running config via ios_facts and saves to backup/
...
Creates 802.1Q subinterfaces and assigns IP addresses
...
Deploys static routes declaratively
...
Access to a Cisco IOS XE lab device or Cisco DevNet Sandbox

Setup

git clone https://github.com/YoYoYoWhatIsUp/network-automation.git
...
Breadth — I can work with multiple automation tools, not just one script style



Ops mindset — Scripts cover real tasks: backup, compliance, monitoring, and config push



Structured output — Results are saved as JSON, not just printed to the terminal



Growth path — From basic Netmiko scripts → Ansible playbooks → Nornir/NAPALM framework



About the Author

GitHub: @YoYoYoWhatIsUp
...
Network Automation Engineer · Network DevOps · DevNet Engineer



Interested in discussing network automation, DevNet, or entry-level network engineering roles. Feel free to reach out via email or LinkedIn.
